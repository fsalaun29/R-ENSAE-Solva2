...
$$ \sqrt{n} (\hat{r}_n - r) \underset{n \to +\infty}{\overset{(d)}{\longrightarrow}} \mathcal{N}(0,1), \quad \hat{r}_n = \frac{1}{2} \ln \left( \frac{1+\hat{\rho}_n}{1-\hat{\rho}_n} \right) = \mathrm{arctanh}(\hat{\rho}_n), \quad r = \mathrm{arctanh}(\rho). $$
...
$$ \sqrt{n} \frac{\hat{\rho}_n - \rho}{1-\rho^2} \underset{n \to +\infty}{\overset{(d)}{\longrightarrow}} Y $$
...
$\hat{\rho}_n \underset{n \to +\infty}{\overset{p.s.}{\longrightarrow}} \rho$
car
...
$S_2$, de caluler $S$ et ensuite d'appliquer la formule:
...
difficulté. L'estimation de la densité de $S$ est représentée dans le
...
$\hat{q}_{0,995} = \hat{VaR}_{99,5\%}$ est obtenue par interpolation
...
L'estimation de la densité de $S$ est représentée dans le graphique
ci-dessous:
...
L'estimation de cette densité au point
$\hat{q}_{0,995} = \hat{VaR}_{99,5\%}$ est égale à :
...
suivante l'ensemble de nos résultats.
...
(inspiré du cours) contenant une boucle afin d'accélérer l'exécution.
...
Nous avons lancé un \emph{microbenchmark} permettant de répéter 100 fois
la génération de $n_0 = 10^2$ simulations de $S_1$ à partir de ces deux
méthodes. Il ressort que l'usage de C permet effectivement d'accélérer
le code, via un gain moyen d'environ 20 millisecondes. Nos expériences
ont montré que cet écart s'accroît lorsque le nombre de simulations $n$
fixé est plus important, ce qui tend à renforcer l'intérêt de notre
démarche d'optimisation pour $n = 10^5$ simulations.
...
1 pour l'approche exacte.

*Ajout calculs*

Nous devons donc approximer la densité de $S$. Par la méthode des
noyaux, nous obtenons la fonction suivante.
