...
print(c("SCR", SCR_FS_norm))

IC_FS_norm <- SCR_FS(SCR_i_norm, r2)
print(c("Intervalle de confiance", IC_FS_norm))
```
...
plot(density_S_norm, xlab = "S", ylab = "Densité", main = "")
```
...
print(c("SCR", SCR_FS_clayton))

IC_FS_clayton <- SCR_FS(SCR_i_clayton, r2)
print(c("Intervalle de confiance", IC_FS_clayton))
```
...
plot(density_S_clayton,xlab = "S", ylab = "Densité", main = "")
```
...
Afin de comparer les deux méthodes, nous présentons dans la table suivante l'ensemble de nos résultats.
```{r, results="asis"}
df_comp <- data.frame("Formule standard" = c(SCR_FS_norm, SCR_FS_clayton), "Modèle exact" = c(SCR_norm, SCR_clayton), row.names = c("Copule gaussienne", "Copule de Clayton"))
...
Plusieurs approches ont été tentées afin d'optimiser le temps d'exécution ainsi que l'usage de mémoire vive. Une telle démarche s'avère nécessaire au vu du grand nombre de simulations à générer. Tout d'abord, nous avons testé une fonction R sans boucle utilisant l'instruction \texttt{sapply}. Ensuite, nous avons utilisé un code C (inspiré du cours) contenant une boucle afin d'accélérer l'exécution.
...
Nous avons lancé un \emph{microbenchmark} permettant de répéter 100 fois la génération de $n_0 = 10^2$ simulations de $S_1$ à partir de ces deux méthodes. Il ressort que l'usage de C permet effectivement d'accélérer le code, via un gain moyen d'environ 20 millisecondes. Nos expériences ont montré que cet écart s'accroît lorsque le nombre de simulations $n$ fixé est plus important, ce qui tend à renforcer l'intérêt de notre démarche d'optimisation pour $n = 10^5$ simulations.
